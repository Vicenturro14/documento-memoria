...
Esta se separa en cinco partes, la primera corresponde a los cambios de las interfaces
respecto a los mockups propuestos en la sección \ref{sec:mockups}. La segunda parte es sobre
la implementación relacionada con los estudiantes del ramo Trabajo de Título, también llamado F.
La tercera parte corresponde a la asignación de comisiones examinadoras. La cuarta parte corresponde a la
exportación de comisiones examinadoras. Por último, la quinta parte corresponde al despliegue a producción.

\section{Cambios en interfaces}
En esta sección se mencionarán los cambios de las interfaces respecto a las interfaces
diseñadas en la sección \ref{sec:mockups} junto con su justificación.

La primera diferencia que se puede ver entre los diseños y la implementación de
interfaces es que en la barra de navegación de los diseños se muestran las pestañas
relacionadas con Trabajo de Título junto al resto de pestañas del módulo Introducción al Trabajo de Título,
mientras que en la implementación actual estas pestañas se encuentran separadas, puesto
que se creó el módulo Trabajo de Título para diferenciar las funcionalidades del ramo Trabajo de Título
y evitar que la barra de navegación se llene de pestañas, como se muestra en la figura
\ref{fig:barra_navegacion_trabajo_titulo}. En la sección \ref{sec:modulo_trabajo_titulo} se explica
la implementación de esta separación con más detalle.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.4\textwidth]{imagenes/implementaciones/selector_seccion_trabajo_titulo.png}
    \caption{Captura de pantalla de la barra de navegación con pestañas de Trabajo de Título.}
    \label{fig:barra_navegacion_trabajo_titulo}
\end{figure}

Una segunda diferencia está en el botón para exportar las comisiones asignadas al SSM,
que en los mockups se muestra con el texto \textit{Publicar}, mientras que en la
implementación se muestra con el texto \textit{Exportar}. Este cambio se hizo, ya que
exportar es la acción que se lleva a cabo al presionarlo. Otra diferencia se encuentra
en el gráfico de carga de evaluadores, puesto en los los mockups se muestra una leyenda
y no se muestran etiquetas en el eje X. En cambio, en la implementación no se muestra la
leyenda y sí se muestran las etiquetas en el eje X. Esto se debe a que la cantidad de
evaluadores es grande, por lo que se priorizó utilizar el espacio horizontal disponible
para que el gráfico sea más legible. Además, en el tooltip que se muestra al posicionar
el cursor sobre una barra del gráfico se muestra qué representa cada color. Respecto a
las etiquetas en el eje X, estas se muestran para que sea más fácil identificar a cada
evaluador. La implementación actual se puede ver en la figura
\ref{fig:cambios_grafico_carga_evaluadores}.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{imagenes/implementaciones/grafico_evaluadores.png}
    \caption{Captura de pantalla del gráfico de carga de evaluadores.}
    \label{fig:cambios_grafico_carga_evaluadores}
\end{figure}

Por último, dos de las interfaces diseñadas en los mockups no se implementaron,
la primera es la confirmación de la exportación de comisiones examinadoras y
la segunda es la incorporación de la comisión asignada a la ficha de un estudiante.
La primera no se implementó, pues no se priorizó ante otras características de la
memoria, ya que más que una funcionalidad, es un aspecto de usabilidad que se puede
mejorar en el futuro. Por otra parte, las comisiones no fueron incorporadas a la ficha
de un estudiante, ya que que esto sería parte de otra memoria que incorpora
cambios a la ficha de los estudiantes.


\section{Estudiantes de Trabajo de Título}

\subsection{Módulo de Trabajo de Título}\label{sec:modulo_trabajo_titulo}
...
muestran en la tabla. Por defecto se muestran los estudiantes del semestre académico
...
asume que los temas de memoria de los estudiantes ya se encuentran registrados en el Sistema de
...
Dado el supuesto anterior, para actualizar el registro de estudiantes de Trabajo de
...
se agrega al estudiante a la lista de estudiantes sin una instancia y se continúa con el
...
se maneja en esta memoria, ya que al momento del desarrollo de este trabajo de título,
había otro tema de memoria sobre la incorporación de un módulo de vía rápida. Mientras
...
que está dividida entre la carga proveniente de las memorias guiadas o coguiadas y la carga
proveniente de las comisiones examinadoras que integra. Es importante destacar que integrar
una comisión examinadora equivale a un punto de carga, mientras que guiar o coguiar una memoria
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.95\linewidth]{imagenes/implementaciones/grafico_evaluadores.png}
    \caption{Captura de pantalla del gráfico de carga de evaluadores.}
...
el modal, pues podía demorar hasta 13 segundos. Esta cantidad de tiempo es inaceptable para
...
era excesivo, ya que por cada llamada se obtenían todos los evaluadores
habílitados y a cada uno se le generaba un string con sus áreas de conocimiento con sus
respectivas subáreas. Este proceso se realizaba para poblar el selector de evaluadores en
...
arreglo de objeto, almacenando su id, nombre, áreas de conocimiento, memorias guiadas,
...
un archivo CSV y enviarlas a través de una petición POST a la API del SSM.
...
secciones \ref{sec:sa:ssm} y \ref{sec:sa:asignacion}, además, la descarga fallaba si una
memoria tenía un coguía externo. Por lo tanto, se cambiaron las
columnas del archivo para que cumplan con los requisitos del SSM, se cambió el separador
al carácter coma y se agregaron las columnas que faltaban, y se cubrió el caso de que una
memoria tuviera un coguía externo. Por último, se agregó la condición de que solo las
memorias con comisión completa sean exportadas. Cabe mencionar que el archivo CSV
que se descarga no puede ser subido directamente al SSM, ya que este solo permite subir
un archivo CSV separados por cursos y desde el Sistema de Titulación se descarga un único
archivo CSV que contiene todas las memorias.


\subsection{Exportación directa al SSM}\label{subsec:exportacion:ssm_directa}
...
de integrantes. El campo coguía puede ser nulo si no hay coguía. Un ejemplo de JSON
...
contengan los campos \texttt{estudiante}, \texttt{tema}, \texttt{guia} e
\texttt{integrantes}. En segundo lugar, se verifica que el contenido no tenga
inconsistencias. Para esto se corrobora que no hayan dos comisiones con el mismo
estudiante y que dentro de cada comisión, el guía, coguía e integrantes sean personas
diferentes. Si cualquiera de estas validaciones falla, se retorna un error
\verb|400 BadRequest| con un mensaje indicando qué validación falló.

En tercer lugar, se registran las memorias y comisiones en la base de datos. Para esto,
se guardan los cursos en el modelo Curso, los estudiantes en el modelo Estudiante,
las memorias en el modelo Memoria y los guías, coguías e integrantes de comisiones en el
modelo Profesor. La inserción de cada uno en la base de datos solo se hace si es que no
existía previamente en esta. Por último, se registran las comisiones en el modelo Miembro,
que relaciona una memoria con un profesor indicando el rol que tiene este en la comisión,
es decir, si es guía, coguía o integrante. En caso de que ya hubieran miembros
previamente definidos, estos se reemplazan por los nuevos. Habiendo terminado el guardado
de datos, se retorna un mensaje de éxito \verb|201 Created|. El código
\ref{lst:guardado_comisiones} muestra el código del guardado de las comisiones sin las
validaciones.
...
debe buscar por nombre, pues solo este atributo se define como único. Además, al guardar un
profesor o estudiante, se normalizan sus nombres, es decir, que todas las
...
y ``Juan-bastián'' (Bastián pasa a minúscula) respectivamente, lo cual puede llevar a estudiantes o
profesores duplicados en la base de datos.

Para solucionar este problema, se implementó la
función \verb|nombre_normalizado|, visible en el código \ref{lst:nombre_normalizado}, para normalizar
los nombres. Se hizo una función a parte para realizar la normalización, ya que este comportamiento
es requerido en más de una parte del código, en particular en el controlador de importación de
comisiones por API y en el procesamiento de archivos CSV subidos por el usuario. Se implementó
esta función a modo de parche en vez de modificar los modelos Profesor y Estudiante, ya que
no se conocía el impacto que podría tener la modificación en el SSM, pues al no ser el
sistema principal de esta memoria, no se conocía en profundidad y se prefirió cambiar lo menos posible.
...
\begin{lstlisting}[language=Python, escapechar=|, label={lst:nombre_normalizado}, caption={Función para normalizar nombres.}]
def nombre_normalizado(nombre: str):
    """
    Retorna un nombre normalizado, es decir, con la primera letra de cada palabra en may|\'u|scula.
    """
...
API, para que no cualquier persona pueda enviar datos al SSM. Con este
...
el Sistema de Titulación puede autenticarse y hacer peticiones POST al endpoint de la
...
se adaptó el SSM para que los casos en los que la fecha de entrega sea \verb|None|, esto
no cause errores y que en la interfaz aparezca \verb|SIN DEFINIR| en rojo y negrita, para
que sea notorio que se requiere asignar una fecha de entrega, como se puede ver en la figura
...
\begin{figure}[ht]
    ...
    \begin{subfigure}[b]{0.5\textwidth}
        ...
        \caption{Notificación de cursos sin fecha de entrega}
...
Como el SSM guarda las memorias distinguiéndolas por el curso de Trabajo de Título en
que se encuentra inscrito el estudiante de cada memoria y no solo por periodo académico,
como se hacía en el Sistema de Titulación, se tuvo que agregar esta información a este
último sistema. Para esto se creó el modelo Curso que representa a una sección de un ramo
en un periodo académico específico y al modelo MemoriaEnF se le agregó un atributo de curso,
indicando el curso al que pertenece la memoria.

La incorporación del modelo Curso al Sistema de Titulación implicó que se tuviera que
reimplementar el comando \verb|upd_estudiantes_f|, que es el encargado de obtener los
estudiantes cursando Trabajo de Título junto a sus memorias. Esta reimplementación
agrega la lógica para obtener los cursos de Trabajo de Título en los que se encuentra
inscrito cada estudiante. Con estos cambios, el Sistema de Titulación tiene la
información necesaria para exportar las comisiones examinadoras al SSM

La exportación de comisiones desde el Sistema de Titulación se ejecuta al presionar el
botón \verb|Exportar| en la vista del listado de comisiones. Este botón realiza una
llamada al endpoint \verb|/comisiones/exportar/<periodo_id>|, donde \verb|<periodo_id>|
es el id del periodo académico que se desea exportar. El controlador de este endpoint se
puede apreciar en el código \ref{lst:export_comisiones_view}, y se encarga de llamar al
comando \verb|export_comisiones_ssm|, que tiene la tarea de realizar la exportación de
comisiones, y luego redirigir al usuario al listado de comisiones. Si el comando arroja
un error, se envía un mensaje de error al usuario. Por otro lado, si todo sale bien, se
envía un mensaje de éxito al usuario. Cabe destacar que es necesario redirigir al usuario
para que el mensaje de éxito o error se muestre en la interfaz gráfica, puesto que así
está diseñado el sistema de mensajes en el Sistema de Titulación.
...
\section{Despliegue}
El último de los objetivos específicos definidos en la sección \ref{sec:intro:objetivos}
es el despliegue del sistema en el servidor de producción y en esta sección se describen
los pasos que se siguieron para llevar a cabo el despliegue tanto del Sistema de Titulación
como del SSM con los cambios realizados.

Como se mencionó en la sección \ref{sec:intro:despliegue}, el despliegue de la herramienta
fue realizado en los servidores del DCC para que pueda ser utilizada por la coordinación
de titulación. Para ello, durante todo el desarrollo del sistema se utilizaron contenedores
de Docker con Docker Compose para coordinar la creación de los contenedores de la base de datos
Postgres y el servidor web Django, facilitando el despliegue. El uso de Docker Compose
no es algo novedoso incorporado por esta memoria en el Sistema de Titulación y el SSM, ambos sistemas
ya lo utilizaban.

El primer despliegue del Sistema de Titulación se realizó en un servidor de test del DCC,
con el objetivo de realizar la evaluación. En esa instancia no se desplegó el SSM, ya que
no se habían realizado cambios en el SSM hasta ese momento. Luego de la evaluación, se
aplicaron los cambios sugeridos en la retroalimentación dada por la coordinadora de titulación
y se implementó la exportación directa de comisiones al SSM descrita en la sección
\ref{subsec:exportacion:ssm_directa}. Con estos cambios, se procedió a realizar un segundo
despliegue en los servidores de test. Esta vez, se incluyó tanto el Sistema de Titulación
como el SSM con los cambios realizados. Se utilizó este despliegue para probar las optimizaciones
realizadas al módulo de Comisiones Examinadoras, la exportación directa de comisiones al
SSM y los cambios realizados en el SSM. Luego de probar junto a Luis Herrera, Ingeniero
del área de TI del DCC, se desplegó el Sistema de Titulación en el servidor de producción.
Por último, se mostró el despliegue del SSM en el servidor de test a la jefa docente
Jocelyn Simmonds, quien está a cargo de este sistema. Luego de probarlo, ella pidió que
se subieran los cambios al servidor de producción para utilizarlo en la gestión de
memorias del semestre de primavera 2025. Por lo tanto, los cambios realizados en el
Sistema de Titulación y el SSM se encuentran desplegados en el servidor de producción.