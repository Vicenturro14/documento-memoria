...
corresponen a Trabajo de Título.


\section{Sistema de Monitoreo de Memorias}\label{sec:sa:sistema}

El Sistema de Monitoreo de Memorias (SMM), es un sistema desarrollado por Matías Rivas
Aguilera en 2024 como su memoria para optar al título de Ingeniero Civil en Computación
\cite{SistemaMonitoreoMemorias}. En la figura \ref{fig:smm_principal} se puede apreciar
la vista principal del SMM. Este sistema se encuentra en producción desde julio de 2025 y
permite a la jefatura de estudios monitorear los plazos de entrega de los informes 
finales de Trabajo de Título y gestionar la corrección de los mismos. Para realizar estas
el sistema requiere saber cuales son las comisiones examinadoras asignadas a cada trabajo
de título y el método que se utiliza para obtenerlos es que el usuario los ingrese, por lo
que el sistema permite agregar, eliminar y modificar memorias, estudiantes, profesores y
miembros de comisiones examinadoras. El ingreso de esta información es realizado por la 
...
\begin{figure}[ht]
    ...
    \caption{Vista principal del SMM que muestra un listado de memoristas.}
    \label{fig:smm_principal}
\end{figure}

El SMM ofrece dos formas de ingresar miembros de comisiones examinadoras. La primera es de
...
\begin{figure}[ht]
    ...
    \caption{Formulario de subida de archivo CSV del SMM.}
    \label{fig:smm_csv}
\end{figure}

La principal limitación del SMM respecto a la asignación de comisiones examinadoras es
...
se puede notar que el SMM no está diseñado para asignar integrantes a comisiones 
...
\begin{figure}[ht]
    ...
    \caption{Formulario del SMM para agregar a un integrante a la comisión de una 
...
examinar las memorias. Además, cae en la misma falta que el SMM, ya que tampoco se tiene
...
equipo del curso CC5401 Ingeniería de Software II durante el semestre de otoño de 2025.
Se trata de una primera versión de una extensión del sistema de titulación del DCC que 
...
examinadoras asignadas al SMM, ya que contiene toda la información requerida por este 
...
caracter \texttt{;} (punto y coma) como separador, mientras que el SMM requiere que el 
caracter \texttt{,} (coma) sea el separador.
...
lugar, el sistema no se encuentra directamente integrado con el SMM.