...
En este capítulo se abordarán el modelo de datos de la herramienta desarrollada y el
diseño de esta herramienta. En la sección \ref{sec:modelo_datos} se describirá el modelo
de datos del sistema de titulación y los cambios por los que este pasó. Mientras que en la
sección \ref{sec:disenno} se describirá el diseño de la herramienta desarrollada.
...
se describirán los cambios que se planearon realizar inicialmente. Por último, se describirán los cambios
al modelo de datos que surgieron durante el desarrollo de esta memoria.
...
entidades son relevantes para el módulo de asignación de comisiones examinadoras, por lo
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.77\linewidth]{imagenes/diagramas/titulacion.png}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.73\linewidth]{imagenes/diagramas/comisiones.png}
...
A continuación se detallará las modificaciones realizadas al modelo de datos que no fueron
planificadas inicialmente, por cada módulo y la razón por la que se realizaron.
...
de un ramo en un periodo académico específico. Esta entidad tiene los atributos periodo,
número de sección, código del ramo, nombre del ramo, id del ramo e id del curso. Estos
dos últimos corresponden a identificadores dentro de la API de UCampus, que es de donde
se obtienen los cursos de un periodo académico. Se agregó esta entidad para identificar
el curso al que pertenece una memoria y así poder exportar las comisiones examinadoras
asignadas al Sistema de Seguimiento de Memorias, puesto que en tal sistema se separan las
memorias por curso.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.4\linewidth]{imagenes/diagramas/modelo_docencia_nuevo.png}
...
Esta entidad representa una memoria en el ramo de Trabajo de Título y tiene los atributos
estudiante, comision, tema, periodo, curso, tipo de memoria, estado de defensa y
memoriaEnE. Esta última es una referencia a la entidad Propuesta, que corresponde a
la misma memoria, pero en el ramo de Introducción al Trabajo de Título.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\linewidth]{imagenes/diagramas/modelo_trabajotitulo_comision_nuevo.png}
...
\begin{itemize}
    \item Titulación
          \begin{itemize}
              \item Se agrega atributo tema a Propuesta
          \end{itemize}
    \item Comisiones
          \begin{itemize}
              \item Se elimina AlumnoCursandoMemoria
              \item Se elimina atributo solicitud de Comision
              \item No se incluyó la entidad ComisionBorrador
          \end{itemize}
    \item Docencia
          \begin{itemize}
              \item Se agrega entidad Curso
          \end{itemize}
    \item Trabajo de Titulo
          \begin{itemize}
              \item Se crea módulo de Trabajo de Titulo
              \item Se crea entidad MemoriaEnF
          \end{itemize}
\end{itemize}
