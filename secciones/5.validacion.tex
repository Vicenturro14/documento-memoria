...
Respecto a las personas que participaron de la evaluación, se consideró solo a la
...
Por lo tanto, los resultados obtenidos son más realistas, tanto en la eficiencia
percibida por la coordinadora, como en la ocurrencia de  posibles errores.
...
por medio de la API, por lo que esto no se evaluó en esa instancia.
...
Además, indicó que el gráfico de carga tiene mucho margen superior, perdiendo espacio
que podría usarse para ampliar el gráfico. Por último, el SSM recibe un archivo CSV
con la asignación de comisiones examinadoras por cada sección de Trabajo de Título,
pero el Sistema de Titulación genera un único archivo CSV con las comisiones examinadoras
para todos los estudiantes de Trabajo de Título de un periodo.
...
examinadoras. Además, al momento de la evaluación, el SSM no estaba siendo utilizado por la
coordinación de estudios, por lo que el archivo CSV generado sí pudo ser utilizado para comunicar
...
\section{Desarrollo y Pruebas Posteriores a la Validación}
Luego de la validación y la optimización mencionada en la sección anterior, se procedió
a agregar la exportación de comisiones examinadoras al SSM por consulta POST, realizando
cambios tanto en el Sistema de Titulación como en el SSM. La implementación de esta
funcionalidad se describe en la sección \ref{subsec:exportacion:ssm_directa}.
Al tener la exportación desarrollada, se desplegaron ambos sistemas en el servidor de test,
para probar la funcionalidad. Se obtuvo como resultado que la exportación se realizó correctamente,
mostrándose las memorias junto a sus respectivas comisiones en la interfaz principal del SSM.

Otra prueba realizada al SSM fue la de utilizar el archivo CSV generado en la evaluación por el
Sistema de Titulación, para verificar que la exportación se realizaba correctamente. Para esto,
se utilizó un script de Python para separar el archivo por secciones del ramo Trabajo de Título,
puesto que el SSM requiere que se envíe la información de cada sección por separado. Como resultado,
se pudo verificar que el SSM recibía la información correctamente y la mostraba en la interfaz
principal. En la figura \ref{fig:ssm_exportacion} se muestra la captura de pantalla del SSM luego
de subir las comisiones examinadoras por CSV.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{imagenes/validacion/resultado_ssm.png}
    \caption{Captura de pantalla del SSM mostrando las comisiones examinadoras subidas por CSV}
    \label{fig:ssm_exportacion}
\end{figure}

Los resultados obtenidos son positivos, ya que a pesar de que fue necesario separar el archivo CSV
con un script de Python, el SSM recibía la información correctamente y la mostraba en la interfaz
principal. Por lo tanto, el formato del archivo CSV generado por el Sistema de Titulación funciona
correctamente para la exportación de comisiones examinadoras al SSM.
